...
- (id)initWithTitle:(NSString *)title message:(NSString *)message timeout:(NSTimeInterval)timeout dismissible:(BOOL)dismissible rect:(CGRect)rect
{
    ...
    CGSize messageSize;
//    if (IS_IOS7_AND_UP)
        messageSize= [message boundingRectWithSize:constrainedSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:MESSAGE_FONT_SIZE]} context:nil].size;
//    else{
...
- (id)initWithTitle:(NSString *)title message:(NSString *)message timeout:(NSTimeInterval)timeout dismissible:(BOOL)dismissible
{
    ...
    if (IS_IOS7_AND_UP)
        titleSize = [title boundingRectWithSize:constrainedSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:TITLE_FONT_SIZE]} context:nil].size;
    else
    {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
        if ([title respondsToSelector:@selector(sizeWithFont:constrainedToSize:)])
            titleSize = [title sizeWithFont:[UIFont boldSystemFontOfSize:TITLE_FONT_SIZE] constrainedToSize:constrainedSize];
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
            if ([message respondsToSelector:@selector(sizeWithFont:constrainedToSize:)])
                messageSize = [message sizeWithFont:[UIFont systemFontOfSize:MESSAGE_FONT_SIZE] constrainedToSize:constrainedSize];
#pragma clang diagnostic pop
        }
...
- (void)clean{
    ...
    if (window.rootViewController.presentedViewController) {
        for (UIView *elt in [window.rootViewController.presentedViewController.view subviews]){
            if ([elt isKindOfClass:[self class]]){
                [elt removeFromSuperview];
            }
        }
    }
    else {
        for (UIView *elt in [window.rootViewController.view subviews]){
            if ([elt isKindOfClass:[self class]]){
                [elt removeFromSuperview];
            }
        }
...
+ (void)cleanAlerts{
    OLGhostAlertView *alert = [[OLGhostAlertView alloc] initWithTitle:@""];
    [alert clean];
}
