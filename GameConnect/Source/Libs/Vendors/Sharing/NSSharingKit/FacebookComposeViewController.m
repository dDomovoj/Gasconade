//
//  FacebookComposeViewController.m
//  FbProto
//
...
@interface FacebookComposeViewController()
{
    FacebookComposeSheetView *_sheetView;
    FacebookComposeBackgroundView *_backgroundView;
    UIView *_backView;
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        _cornerRadius = 5;
        _sheetView = [[FacebookComposeSheetView alloc] initWithFrame:CGRectMake(0, 0, self.currentWidth - 8, 202)];
    }
    return self;
}
...
- (void)viewDidLoad
{
    ...
    _backgroundView = [[FacebookComposeBackgroundView alloc] initWithFrame:self.view.bounds];
    ...
    if (!_attachmentImage)
        _attachmentImage = [UIImage imageNamed:@"NSShareKitResources.bundle/URLAttachment"];
...
#pragma mark -
#pragma mark FacebookComposeSheetViewDelegate

- (void)cancelButtonPressed
{
    if (_delegate && [_delegate respondsToSelector:@selector(composeViewController:didFinishWithResult:)]) {
        [_delegate composeViewController:self didFinishWithResult:FacebookComposeCancelled];
    }
    if (_completionHandler) {
        _completionHandler(FacebookComposeCancelled);
    }
...
{
    if (_delegate && [_delegate respondsToSelector:@selector(composeViewController:didFinishWithResult:)]) {
        [_delegate composeViewController:self didFinishWithResult:FacebookComposeResultPosted];
    }
    if (_completionHandler) {
        _completionHandler(FacebookComposeResultPosted);
    }
    [self dismissViewControllerAnimated:YES completion:nil];
}
