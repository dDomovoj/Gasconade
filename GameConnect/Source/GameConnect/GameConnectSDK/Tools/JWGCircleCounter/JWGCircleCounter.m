...
#define JWG_TIMER_INTERVAL 0.01 // 1/100 FPS
...
{
    [self startWithSeconds:seconds usingTimer:YES];
}
...
- (void)update
{
    if (self.isRunning)
    {
        numAdjustedSecondsCompleted += numAdjustedSecondsTotal * (JWG_TIMER_INTERVAL / numSeconds);
        ...
        if (numAdjustedSecondsCompleted >= numAdjustedSecondsTotal)
        {
            ...
            if ([self.delegate respondsToSelector:@selector(circleCounterTimerDidFire:onTime:)])
                [self.delegate circleCounterTimerDidFire:self onTime:(numAdjustedSecondsTotal - numAdjustedSecondsCompleted)];
        ...
        {
//            NSLog(@"Seconds : %lu ON %lu", (unsigned long)numAdjustedSecondsCompleted, (unsigned long)numAdjustedSecondsTotal);
            // in progress
...
-(void)startWithSeconds:(NSInteger)seconds usingTimer:(BOOL)timerEnable
{
    if (seconds < 1)
        return;
...
-(void)updateWithSecondsRemaining:(CGFloat)secondsRemaining
{
    numAdjustedSecondsCompleted = secondsRemaining * JWG_SECONDS_ADJUSTMENT;
