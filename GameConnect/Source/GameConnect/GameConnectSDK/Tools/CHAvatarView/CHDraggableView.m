...
-(void)myInit
{
    self.bubbleType = eCHDraggableViewSimple;
}
...
{
    NSLog(@"[TOUCH BEGAN");
//    timerLongPress = [NSTimer scheduledTimerWithTimeInterval:2 target:self selector:@selector(longPressEvent) userInfo:nil repeats:NO];
...
{
    _moved = YES; // TRICKS
    if (self.delegate && [self.delegate respondsToSelector:@selector(draggableViewLongTouched:)])
        [self.delegate draggableViewLongTouched:self];
}
